...
def extract_first_name(resume_text):
    # Use a simple regex to capture the first word (assumed to be the first name)
...
if submitted and resume_file is not None and len(job_description) > 100:
    try:
        ...
        display_results(result, position_titles)  # Pass position_titles as an argument
...
with st.form(key='feedback_form'):
    ...
    if submit_feedback:
        ...
        save_feedback_data(feedback_data)  # Save feedback data to file
